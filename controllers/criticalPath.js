...
const criticalPath = async (project_id, snapshot_date) => {
    ...
    try{
        ...
        for(let i=0; i<path_array.length; i++){
            let query = `select task_id from gantt_chart where 
            uid = '${path_array[i]}' 
            and project_uid = '${project_id}' and snapshot_date = '${snapshot_date}';`
            let taskID = await tempConnection.query(query);
            await tempConnection.query(`update gantt_chart set on_cp = 1 where task_id = ${taskID[0].task_id};`);
        }
    }
    catch(error){
